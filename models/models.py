...
class Model(nn.Module):
    ...
    def __init__(self, name=None, column_names=None, num_epochs=100, channels=16, num_layer=2, embedding=8, gating=0.,
                 ...
                 evaluate_train=True, verbose=True, full_data_cuda=True, attention_head = 0):
        self.name = name
...
class Model_regression(nn.Module):
    ...
    def fit(self, X, y, adj=None):
        ...
        self.y = y
        self.setup_layers()
        x_train, x_valid, y_train, y_valid = sklearn.model_selection.train_test_split(X, y, shuffle=True, train_size=self.train_valid_split, test_size=1-self.train_valid_split, random_state=self.seed)
        ...
        for epoch in range(0, self.num_epochs):
            ...
            if self.evaluate_train:
                ...
                y_hat = np.concatenate(res)
                #auc['train'] = self.metric(y_true, np.argmax(y_hat, axis=1))
                rmse['train'] = np.sqrt(self.metric(y_true, y_hat))
            ...
            y_hat = np.concatenate(res)
            #auc['valid'] = self.metric(y_valid, np.argmax(y_hat, axis=1))
            rmse['valid'] = np.sqrt(self.metric(y_valid, y_hat))
            
            patience = patience - 1
            if patience == 0:
                break
            if (max_valid < rmse['valid']) and epoch > 5:
                max_valid = rmse['valid']
                patience = self.start_patience
                self.best_model = self.state_dict().copy()
            if self.verbose:
                print("epoch: " + str(epoch) + ", time: " + "{0:.2f}".format(time.time() - start) + ", valid_metric: " + "{0:.2f}".format(rmse['valid']) + ", train_metric: " + "{0:.2f}".format(rmse['train']))
            if self.scheduler:
                scheduler.step()
        if self.verbose:
            print("total train time:" + "{0:.2f}".format(time.time() - all_time) + " for epochs: " + str(epoch))
